...
public class ImageFormatTests
{
    ...
    [Test]
    public void ImageFormat_EncodeTGA_DecodeTGA_ProducesExactData()
    {
        const int kDimension = 64;
        const int kLength = kDimension * kDimension;
        ...
        var encoded = CaptureEncodeTGA.Encode(ArrayUtilities.Cast<byte>(data), kDimension, kDimension, GraphicsFormat.R8G8B8A8_UNorm);

        int width = 0, height = 0;
        var decoded = ArrayUtilities.Cast<Color32>(CaptureEncodeTGA.Decode(ArrayUtilities.Cast<byte>(encoded), ref width, ref height));
        ...
        for (var i = 0; i < kLength; ++i)
        {
            var cs = data[i];
            var cd = decoded[i];
            int rd = Math.Abs((int)cs.r - (int)cd.r);
            int gd = Math.Abs((int)cs.g - (int)cd.g);
            int bd = Math.Abs((int)cs.b - (int)cd.b);
            int ad = Math.Abs((int)cs.a - (int)cd.a);
            if (rd > kDeviation || gd > kDeviation || bd > kDeviation || ad > kDeviation)
                ++count;
        }
