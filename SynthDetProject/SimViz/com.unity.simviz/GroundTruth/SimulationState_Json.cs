...
namespace UnityEngine.SimViz
{
    partial class SimulationState
    {
        ...
        void WritePendingCaptures(bool flush = false, bool writeCapturesFromThisFrame = false)
        {
            ...
            List<PendingCapture> pendingCapturesToWrite = new List<PendingCapture>(m_PendingCaptures.Count);
            var frameCountNow = Time.frameCount;
            for (int i = 0; i < m_PendingCaptures.Count; i++)
            {
        ...
        void WritePendingMetrics(bool flush = false)
        {
            ...
            List<PendingMetric> pendingMetricsToWrite = new List<PendingMetric>(m_PendingMetrics.Count);
            m_SerializeMetricsSampler.Begin();
            for (int i = 0; i < m_PendingMetrics.Count; i++)
            {
