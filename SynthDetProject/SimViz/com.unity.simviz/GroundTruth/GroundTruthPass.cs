...
{
    public abstract class GroundTruthPass : CustomPass, IGroundTruthPass
    {
        ...
        protected void EnsureActivated()
        {
            if (!m_IsActivated)
            {
                ...
                labelSetupSystem?.ActivatePass(this);
                m_IsActivated = true;
            }
        }
        ...
        {
            var labelSetupSystem = World.DefaultGameObjectInjectionWorld?.GetExistingSystem<GroundTruthLabelSetupSystem>();
            labelSetupSystem?.DeactivatePass(this);
        }
        ...
        {
            var shaderPasses = new ShaderTagId[]
            {
